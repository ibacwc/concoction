...
char* WrapText(char* text, int size, int lineSize){
	...
	tmp[0]='\0';
	for(int i = 0; i<textWidth/size;++i){
		if(i%charsInLine == 0 ){
			if (i !=0) strcat(retVal, "\n");
...
int Init(){
	...
	char* brokenText = WrapText(tutorialText, 18, PAGE_WIDTH);
...
void RenderPage(Page* page){
	...
	DrawTextEx(terminus, page->text, (Vector2){page->position[0]+5,page->position[1]+10},18, 0,BLACK);
}
void Render(){
	BeginDrawing();
